  ...
  return nodes.map((node, i) => {
    ...
    if (node.tag === conditionals[0]) {
      ...
      if (!node.attrs && !node.attrs.condition) {
        throw new Error('the "if" tag must have a "condition" attribute')
      }
      ...
      while (conditionals.slice(-2).indexOf(nextTag.tag) > -1) {
        ...
        if (nextTag.tag === conditionals[1]) {
          ...
          if (!nextTag.attrs && !nextTag.attrs.condition) {
            throw new Error('the "elseif" tag must have a "condition" attribute')
          }
      ...
      const expression = ast.reduce((m, e, i) => {
        m += e.statement
      ...
      // run the condition
      if (vm.runInContext(node.attrs.condition, ctx)) {
        nodes.splice(i, 1, ...node.content)
        return ''
      }
    }
